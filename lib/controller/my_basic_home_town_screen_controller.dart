...
class MyBasicHomeTownScreenController extends GetxController {

  RxBool isLoading = false.obs;
  RxInt successStatus = 0.obs;

  GlobalKey<FormState> formKey = GlobalKey<FormState>();
  TextEditingController homeTownFieldController = TextEditingController();
  ...
  Future<void> doneButtonClick() async {
    if(formKey.currentState!.validate()) {
      ...
      String homeTown = homeTownFieldController.text.trim();
      log('homeTown :$homeTown');
      await saveHomeTownFunction(
        key: AppMessages.homeTownApiText,
        value: homeTown,
      );
    }
  }

  Future<void> saveHomeTownFunction(
      {required String key, required String value}) async {
    isLoading(true);
    String url = ApiUrl.completeSignUpApi;
    log('saveHomeTownFunction Api Url : $url');
    ...
    try {
      ...
      response.stream.transform(utf8.decoder).listen((value1) async {
        ...
        if (successStatus.value == 200) {
          ...
          log('saveHomeTownFunction Else');
        }
      });
    } catch (e) {
      log('saveHomeTownFunction Error :$e');
      rethrow;
    }
  }
